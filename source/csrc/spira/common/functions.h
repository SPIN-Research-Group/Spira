...
template <typename T>
SPIRA_FORCEINLINE SPIRA_HOST_DEVICE constexpr T DivCeil(const T &a,
                                                          const T &b) {
  return (a + b - 1) / b;
}

template <typename T>
SPIRA_FORCEINLINE SPIRA_HOST_DEVICE constexpr T DivFloor(const T &a,
                                                           const T &b) {
  return a / b;
}
...
struct IterateImpl {
  ...
  SPIRA_FORCEINLINE SPIRA_HOST_DEVICE static void Evaluate(
      CallableT callable) {
    callable(T_BEGIN);
...
struct IterateImpl<T, T_BEGIN, T_END, false> {
  template <typename CallableT>
  SPIRA_FORCEINLINE SPIRA_HOST_DEVICE static void Evaluate(CallableT) {}
};
...
SPIRA_FORCEINLINE SPIRA_HOST_DEVICE void Iterate(CallableT callable) {
...
struct CompareCoordinates {
  ...
  SPIRA_FORCEINLINE SPIRA_HOST_DEVICE T operator()(SourceT source,
                                                     TargetT target) {
    T delta = 0;
...
SPIRA_FORCEINLINE SPIRA_HOST_DEVICE constexpr const T FloorLog2(T x) {
  return x == 1 ? 0 : 1 + FloorLog2(x >> 1);
}

template <typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
SPIRA_FORCEINLINE SPIRA_HOST_DEVICE constexpr const T CeilLog2(T x) {
  return x == 1 ? 0 : 1 + FloorLog2(x - 1);
}
