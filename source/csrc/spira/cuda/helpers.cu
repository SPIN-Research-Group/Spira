...
#define SPIRA_CHECK_CUBLAS(stmt, ...)                              \
  do {                                                              \
    auto return_code = (stmt);                                      \
    SPIRA_CHECK(return_code == CUBLAS_STATUS_SUCCESS,              \
...
void MatMul(std::size_t m,           //
  ...
  if (stream != nullptr) {
    ...
    SPIRA_CHECK_CUBLAS(cublasSetStream(context.GetCUBLASHandle(), stream));
  }
  // We need to swap a and b to get C^T = B^T A^T to receive row-major format
  SPIRA_CHECK_CUBLAS(CUBLASDispatcher<FT>::GEMM(
  ...
  if (stream != nullptr) {
    SPIRA_CHECK_CUBLAS(cublasSetStream(context.GetCUBLASHandle(), old_stream));
  }
}
...
void BatchedMatMul(std::size_t b,           //
  ...
  SPIRA_CHECK_CUBLAS(CUBLASDispatcher<FT>::GEMMStridedBatched(
      context.GetCUBLASHandle(),                    // handle
      is_b_transposed ? CUBLAS_OP_T : CUBLAS_OP_N,  // transa
      is_a_transposed ? CUBLAS_OP_T : CUBLAS_OP_N,  // transb
      n,                                            // m
...
#define SPIRA_EXPLICIT_INSTANTIATE(FT)                                        \
...
SPIRA_EXPLICIT_INSTANTIATE(half);
SPIRA_EXPLICIT_INSTANTIATE(float);
SPIRA_EXPLICIT_INSTANTIATE(double);
#undef SPIRA_EXPLICIT_INSTANTIATE
