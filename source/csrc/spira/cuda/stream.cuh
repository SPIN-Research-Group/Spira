...
class Stream final {
 public:
  Stream() : stream_(), is_owned_(true) {
    SPIRA_CHECK_CUDA(cudaStreamCreate(&stream_));
  }

  explicit Stream(unsigned int flags) : stream_(), is_owned_(true) {
    SPIRA_CHECK_CUDA(cudaStreamCreateWithFlags(&stream_, flags));
  }
  ...
  void Close() {
    if (is_owned_) {
      SPIRA_CHECK_CUDA(cudaStreamDestroy(stream_));
      is_owned_ = false;
    }
  }
  ...
  void Synchronize() { SPIRA_CHECK_CUDA(cudaStreamSynchronize(stream_)); }
