...
#ifdef SPIRA_DEBUG
    std::cerr << "[SPIRA] Create a new buffer with " << buffer_size
...
BufferPool::~BufferPool() noexcept(false) {
  ...
  if (!buffers_.empty()) {
    std::cerr
        << "[SPIRA] Destructing BufferPool with active buffers on stream "
        << stream_ << std::endl;
    for (const auto &buffer : buffers_) {
      std::cerr << "[SPIRA] Found Buffer(used=" << buffer->used()
                << ", size=" << buffer->num_entries() << ")" << std::endl;
    }
    SPIRA_ERROR("Cannot destruct buffer pool with actively allocated buffers");
  }
...
#ifdef SPIRA_DEBUG
  std::cerr << "[SPIRA] Create a buffer with stream " << stream << std::endl;
#endif
...
void BufferPool::SetGrowth(float growth) {
  SPIRA_CHECK(growth >= 1, "growth has to be larger than 1 but found ",
               growth);
  this->growth_ = growth;
}
void BufferPool::SetPageSize(std::size_t page_size) {
  SPIRA_CHECK(page_size > 0, "page_size has to be larger than 0 but found ",
               page_size);
...
void *BufferPool::Buffer::Acquire(std::size_t size, std::size_t alignment) {
  ...
  SPIRA_CHECK(aligned_size <= idle(), "Acquire ", aligned_size, " bytes (",
...
void BufferPool::Buffer::Release(void *data) {
  SPIRA_CHECK(!allocated_.empty() && allocated_.back().first == data,
...
void BufferPool::Buffer::SilentRelease(void *data) noexcept {
  if (!allocated_.empty() && allocated_.back().first == data) {
    ...
    std::cerr << "[SPIRA] Warning: Releasing a buffer that is not on the top "
                 "causes memory leak\n";
  }
}
