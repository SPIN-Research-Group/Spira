...
#include "spira/cuda/device/loads.cuh"
#include <cuda_fp16.h>
...

  {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0)
    {
      ...
      if (input_idx != -1)
      {
        ...
        device::global_load<K_ld_factor>(A_loaded, A_ptr_local + input_idx * K_original + ((ax0_ax1_fused_0 * 512 % 16) % K_tile_padded), A_pred_guard);
    ...
    if (threadIdx.y == 0)
    {
      ...
      device::global_load<N_ld_factor>(B_loaded, B_ptr_local, B_pred_guard);
...

    {
      ...
      for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 8; ++ax0_ax1_fused_0)
      {
        ...
        if (input_idx != -1)
        {
          ...
          uint4 A_loaded = make_uint4(0, 0, 0, 0);
          device::global_load<K_ld_factor>(A_loaded, A_ptr_local + (input_idx * K_original) , A_pred_guard);
      ...
      for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 1; ++ax0_ax1_fused_0_1)
      {
        ...
        uint4 B_loaded = make_uint4(0, 0, 0, 0);
        device::global_load<N_ld_factor>(B_loaded, B_ptr_local + (ax0_ax1_fused_0_1 * 16) * N, B_pred_guard); 
