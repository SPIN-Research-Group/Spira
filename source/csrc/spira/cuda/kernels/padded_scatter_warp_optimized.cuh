...
__global__ void PaddedScatterWarpOptimized(
  ...
  const auto gwid = SPIRA_GLOBAL_THREAD_ID(x) / T_WARP_SIZE;
  const auto gwsz = SPIRA_N_GLOBAL_THREADS(x) / T_WARP_SIZE;
  const auto wid = SPIRA_GLOBAL_THREAD_ID(x) % T_WARP_SIZE;
