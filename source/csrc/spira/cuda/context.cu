...
Context::Context(int device, cudaStream_t stream)
  ...
  SPIRA_CHECK(NumDevices() > 0, "No CUDA device is found");
  SPIRA_CHECK(0 <= device && device < NumDevices(),
...
Context::Context(cudaStream_t stream)
  ...
  SPIRA_CHECK_CUDA(cudaGetDevice(&device_));
}
...
int Context::NumDevices() {
  ...
  if (num_devices == nullptr) {
    ...
    SPIRA_CHECK_CUDA(cudaGetDeviceCount(num_devices.get()));
  }
  return *num_devices;
}
...
const cudaDeviceProp &Context::GetDeviceProp() const {
  ...
  if (result.second) {
    SPIRA_CHECK_CUDA(cudaGetDeviceProperties(&result.first->second, device_));
  }
