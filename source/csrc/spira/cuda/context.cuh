...
#ifndef __NVCC__
#error "spira/cuda/common.cuh must be compiled with NVCC"
#endif
...
#define SPIRA_GLOBAL_THREAD_ID(dim) \
  ((blockIdx.dim) * (blockDim.dim) + (threadIdx.dim))
#define SPIRA_N_GLOBAL_THREADS(dim) ((gridDim.dim) * (blockDim.dim))
#define SPIRA_LOCAL_THREAD_ID(dim) (threadIdx.dim)
#define SPIRA_N_LOCAL_THREADS(dim) (blockDim.dim)
#define SPIRA_THREAD_BLOCK_ID(dim) (blockIdx.dim)
#define SPIRA_N_THREAD_BLOCKS(dim) (gridDim.dim)
#define SPIRA_FOR_DIMENSION_AND_INDEX(MACRO, DELIMITER) \
 ...
 public:
  ...
  T ReadDeviceData(const T *data) const {
    ...
    if (stream_ == cudaStreamDefault) {
      ...
      SPIRA_CHECK_CUDA(cudaMemcpyAsync(&result, data, sizeof(T),
  ...
  void ReadDeviceData(const T *device_data, T *host_data,
    ...
    if (stream_ == cudaStreamDefault) {
      SPIRA_CHECK_CUDA(cudaMemcpy(host_data, device_data, sizeof(T) * size,
                                   cudaMemcpyDeviceToHost));
    } else {
      SPIRA_CHECK_CUDA(cudaMemcpyAsync(host_data, device_data,
                                        sizeof(T) * size,
                                        cudaMemcpyDeviceToHost, stream_));
    }
  }
  ...
  void Synchronize() const {
    SPIRA_CHECK_CUDA(cudaStreamSynchronize(stream_));
  }
  void Synchronize(const std::string &hint) const {
    SPIRA_CHECK_CUDA_WITH_HINT(cudaStreamSynchronize(stream_), hint);
  }

  static void DeviceSynchronize() {
    SPIRA_CHECK_CUDA(cudaDeviceSynchronize());
  }

  static void DeviceSynchronize(const std::string &hint) {
    SPIRA_CHECK_CUDA_WITH_HINT(cudaDeviceSynchronize(), hint);
  }
...
#define CHECK_BLOCK_SIZE(dim, id)                                              \
  do {                                                                         \
    if (block_size.dim > device_prop.maxThreadsDim[id]) {                      \
      SPIRA_ERROR("The dimension" #dim " of the block size (",                \
                   block_size.dim, ") is larger than supported (",             \
                   device_prop.maxThreadsDim[id], ")");                        \
    }                                                                          \
    if (block_size.dim == 0) {                                                 \
      SPIRA_ERROR("The dimension " #dim " of the block size cannot be zero"); \
    }                                                                          \
  } while (false)
    SPIRA_FOR_DIMENSION_AND_INDEX(CHECK_BLOCK_SIZE, SPIRA_SEMICOLON);
#undef CHECK_BLOCK_SIZE
    ...
    if (num_threads_per_block > device_prop.maxThreadsPerBlock) {
      SPIRA_ERROR("The number of threads in each block (",
                   num_threads_per_block,
                   ") for the kernel is larger than supported (",
                   device_prop.maxThreadsPerBlock, ")");
    }
    if (num_threads_per_block % device_prop.warpSize != 0) {
      SPIRA_ERROR("The block size (", block_size.x, ", ", block_size.y, ", ",
...
#define GRID_SIZE(dim, id)                                         \
  (std::min(static_cast<CUDADimType>(device_prop.maxGridSize[id]), \
    ...
    dim3 grid_size(SPIRA_FOR_DIMENSION_AND_INDEX(GRID_SIZE, SPIRA_COMMA));
#undef GRID_SIZE

    kernel<<<grid_size, block_size, shared_memory, stream_>>>(
        std::forward<ArgTs>(args)...);
    SPIRA_CHECK_CUDA(cudaGetLastError());
  }
