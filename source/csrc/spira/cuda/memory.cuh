...
template <typename T>
SPIRA_DEVICE T *DynamicSharedMemory() {
...
class Memory {
 public:
  ...
  [[nodiscard]] SPIRA_HOST_DEVICE const T *device_data() const {
    return d_data_;
  }
  [[nodiscard]] SPIRA_HOST_DEVICE T *device_data() { return d_data_; }
  [[nodiscard]] SPIRA_HOST_DEVICE std::size_t size() const { return size_; }
  ...
  void CopyTo(T *target, std::size_t size, std::size_t base = 0,
    ...
    if (stream_ == cudaStreamDefault) {
      ...
      SPIRA_CHECK_CUDA(cudaMemcpyAsync(target, d_data_ + base,
  ...
  void AsyncCopyTo(
      ...
      cudaMemcpyKind kind = cudaMemcpyKind::cudaMemcpyDefault) const {
    SPIRA_CHECK_CUDA(cudaMemcpyAsync(target, d_data_ + base, size * SizeOf<T>,
                                      kind, stream_));
  }
  ...
  void CopyFrom(const T *source, std::size_t size, std::size_t base = 0,
    ...
    if (stream_ == cudaStreamDefault) {
      ...
      SPIRA_CHECK_CUDA(cudaMemcpyAsync(d_data_ + base, source,
                                        size * SizeOf<T>, kind, stream_));
      SPIRA_CHECK_CUDA(cudaStreamSynchronize(stream_));
    }
  }
  ...
  void AsyncCopyFrom(const T *source, std::size_t size, std::size_t base = 0,
                     cudaMemcpyKind kind = cudaMemcpyKind::cudaMemcpyDefault) {
    SPIRA_CHECK_CUDA(cudaMemcpyAsync(d_data_ + base, source, size * SizeOf<T>,
                                      kind, stream_));
  }
  ...
  void Memset(int value, std::size_t size, std::size_t base = 0) {
    if (stream_ == cudaStreamDefault) {
      SPIRA_CHECK_CUDA(cudaMemset(d_data_ + base, value, SizeOf<T> * size));
    } else {
      SPIRA_CHECK_CUDA(
